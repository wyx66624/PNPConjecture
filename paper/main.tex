...
Our second contribution is a presentation-invariance principle. If an ignored
dummy certificate type \(D\) is added, then the accepting-certificate fiber
becomes exactly \(D\) times the original fiber, while the recognized language
is unchanged whenever \(D\) is nonempty. Thus raw witness multiplicity and
unquotiented trace topology are not language invariants. This motivates
Verifier-Invariant Obstruction Theory (VIOT), in which topological data are
formed only after localizing verifier presentations by resource-preserving
simulations, padding, stuttering, and recoding. We formulate a sound
obstruction schema and a barrier-aware program connecting restriction sites,
constructive refuters, convergent gate elimination, proof complexity, and
equivariant topology. The central circuit lower bound remains open.
\end{abstract}
...
The counterexamples, certificate-fiber equivalence, and padding theorem below
are proved and formalized. The VIOT constructions are definitions and
conjectures, not a proof of \(P\ne NP\).
...
basis \cite{LiYang}. Carmosino, Dang, and Jackman show that simplification is
confluent over DeMorgan and \(\{\land,\lor,\oplus\}\) bases but not over
\(\mathcal U_2\) and \(\mathcal B_2\), and connect simplification with
constructive refuters \cite{CDJ}. Improving a linear constant is valuable but
asymptotically insufficient for NP versus \(P/poly\).

\section{Audit of a claimed homological separation}
\label{sec:audit}

The construction in \cite{Tang} takes directed computation paths
\((c_0,\ldots,c_n)\), beginning at an initial configuration and ending at a
terminal configuration, as generators and proposes
\[
 d_n(c_0,\ldots,c_n)=
...
There is a valid directed computation path none of whose ordinary faces is a
valid computation path. Hence the displayed \(d_n\) need not map the stated
group \(C_n\) to \(C_{n-1}\).
\end{theorem}
...
\]
and no transition \(c_0\to c_2\). Designate \(c_0\) as the only initial
configuration and \(c_2\) as the only terminal configuration. Then
\((c_0,c_1,c_2)\) is valid. Its faces are
\[
 (c_1,c_2),\qquad(c_0,c_2),\qquad(c_0,c_1).
\]
The first does not start initially, the second uses a nonexistent transition,
and the third does not end terminally. Thus every summand of
\(d_2(c_0,c_1,c_2)\) can be absent from the stated codomain.
\end{proof}
...
\section{Verifier padding and no-go principles}
\label{sec:padding}

\begin{definition}
For a verifier \(V:X\times W\to\{0,1\}\), define its accepting-certificate
fiber above \(x\) by
\[
 W_x(V)=\{w\in W:V(x,w)=1\}.
\]
For a dummy certificate type \(D\), define
\[
 V_D(x,(d,w))=V(x,w).
\]
\end{definition}

\begin{theorem}[Certificate-fiber product theorem]
For every input \(x\), there is a canonical equivalence
\[
...
\begin{proof}
Map an accepting padded certificate \((d,w)\) to
\((d,w)\), now regarding \(w\) as an element of \(W_x(V)\). The inverse
forgets that regrouping. Both composites are definitionally the identity.
\end{proof}

\begin{theorem}[Padding invariance]
If \(D\) is nonempty, then for every \(x\),
\[
...
\begin{proposition}[Concrete witness-structure counterexample]
There are two verifiers recognizing the same language such that one
accepting-certificate fiber is a subsingleton and the other contains two
distinct elements.
\end{proposition}

\begin{proof}
Let \(X=W=\{*\}\) and let \(V(*,*)=1\). The sole fiber of \(V\) is a
singleton. Pad by \(D=\{0,1\}\). The padded verifier recognizes the same
language, while \((0,*)\) and \((1,*)\) are distinct accepting certificates.
\end{proof}

\begin{corollary}[Raw-presentation no-go principle]
A quantity that changes under ignored nonempty certificate padding does not
factor through the recognized language. In particular, raw witness
multiplicity and any unquotiented invariant that detects the product factor
\(D\) are not intrinsic language-complexity invariants.
\end{corollary}
...
The companion Lean project formalizes the three-face closure counterexample,
the absent many-one reduction, nonadditivity of \(\rho\), failure of the
proposed contraction, the certificate-fiber product equivalence,
verifier-padding equivalence, a concrete same-language/different-witness
counterexample, abstract obstruction soundness, and abstract class-separation
criteria. CI rejects `sorry', `admit', and user-declared `axiom'. These are
formalized auxiliary results; they are not a formalization of \(P\ne NP\).
...
\bibitem{LiYang} J. Li, T. Yang, \(3.1n-o(n)\) circuit lower bounds for explicit functions, STOC, pp. 1180--1193, 2022.
